...
class CogData:
    def __init__(self, cog_name: str):
        ...
        self.__defaults : dict[type[discord.abc.Snowflake] | str, tuple[BuildTable, ...]] = {}
    ...
    def __get_manager(self, model: discord.abc.Snowflake | str) -> 'ModelDataManager':
        ...
        defaults = self.get_registered(type(model) if isinstance(model, discord.abc.Snowflake) else model)
    ...
    def register(self, model_type: type[discord.abc.Snowflake] | str = 'global', *defaults: 'BuildTable') -> None:
    ...
    def get_registered(self, model_type: type[discord.abc.Snowflake] | str = 'global') -> tuple['BuildTable', ...]:
...
class ModelDataManager:
    ...
    def __init__(self, model: discord.abc.Snowflake | str, db_path: Path, *, defaults: Sequence['BuildTable'] = []):
        self.model = model
    ...
    def __get_connection(self, path: Path) -> sqlite3.Connection:
        ...
        with closing(conn.cursor()) as cursor:
            ...
            for default in self.defaults:
                if not default.table_name in tables: # Si la table existe déjà et qu'on ne veut pas réinsérer les valeurs par défaut
                    print(f'-- Initialisation de la table {default.table_name}...')
                ...
                if default.insert_on_reconnect:
                    print(f'-- Réinsertion des valeurs par défaut dans la table {default.table_name}...')
                    cursor.executemany(f'INSERT OR IGNORE INTO {default.table_name} ({", ".join(default.default_values[0].keys())}) VALUES ({", ".join(["?" for _ in default.default_values[0]])})',     
                                        [tuple(d.values()) for d in default.default_values])
        if commit_on_close:
            conn.commit()
...
class BuildDictTable(BuildTable): # Pour les tables simplifiées de type clé/valeur
