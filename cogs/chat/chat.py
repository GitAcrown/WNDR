...
HISTORY_MESSAGES_EXPIRATION = 604800 # secondes
...
class AskChatbotModal(discord.ui.Modal):
    def __init__(self):
        super().__init__(title='Demander au chatbot', timeout=60.0)
        
        placeholder_examples = [
            "Quel animal se trouve sur l'image ?",
            "Quelle est la couleur dominante sur cette photo ?",
            "D'où vient la citation de ce message ?",
            "Peut-tu me traduire ce texte en anglais ?",
            "Quelle est la réponse à cette énigme ?",
            "De quel pays cette personne parle ?"
        ]
        
        self.ask = discord.ui.TextInput(label='Votre demande', min_length=3, max_length=200, placeholder=random.choice(placeholder_examples))
        self.add_item(self.ask)
...
class Chat(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        self.data.register('global', chatbots, users)
        
        # GUILDS
        sessions = dataio.BuildTable(
        ...
        self.data.register(discord.Guild, sessions)
    ...
    async def ask_chatbot_callback(self, interaction: Interaction, message: discord.Message):
        """Callback pour demander une réponse à un chatbot via le menu contextuel."""
